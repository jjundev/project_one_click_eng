...
public final class RefinerStatsStore {
  private static final String PREF_NAME = "refiner_stats_store";
  ...
  public RefinerStatsStore(@NonNull Context context) {
    this.preferences =
        context.getApplicationContext().getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
    this.gson = new Gson();
  }

  public void saveRoundResult(@NonNull RefinerRoundResult result) {
    List<RefinerRoundResult> rounds = getRecentResults();
    rounds.add(0, result);
  ...
  @NonNull
  public List<RefinerRoundResult> getRecentResults() {
    String raw = preferences.getString(KEY_ROUNDS, "");
    if (raw == null || raw.trim().isEmpty()) {
      return new ArrayList<>();
    }
    try {
      Type type = new TypeToken<List<RefinerRoundResult>>() {}.getType();
      List<RefinerRoundResult> parsed = gson.fromJson(raw, type);
